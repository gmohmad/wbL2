...
func TestMySort(t *testing.T) {
	for _, test := range MySortTests {
		output, err := Sort(test.data, test.r, test.n, test.u, test.k)
		...
		if test.expected != output {
			t.Errorf("Failed: expected - %s, got - %s", test.expected, output)
		}
	}
}
